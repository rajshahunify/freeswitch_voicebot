...
sed -i "s|\(external_rtp_ip=\)[^\"]*|\1${EXTERNAL_IP}|g" "${FS_CONF}/vars.xml"
sed -i "s|\(external_sip_ip=\)[^\"]*|\1${EXTERNAL_IP}|g" "${FS_CONF}/vars.xml"
...
if [ -f "${VOICEBOT_DIALPLAN}" ]; then
    sed -i "s|expression=\"^[0-9]*$\"|expression=\"^${VOICEBOT_EXTENSION}$\"|g" "${VOICEBOT_DIALPLAN}"
fi

# Update the public context route
PUBLIC_ROUTE="${FS_CONF}/dialplan/public/01_voicebot_route.xml"
if [ -f "${PUBLIC_ROUTE}" ]; then
    sed -i "s|expression=\"^[0-9]*$\"|expression=\"^${VOICEBOT_EXTENSION}$\"|g" "${PUBLIC_ROUTE}"
    sed -i "s|data=\"[0-9]* XML default\"|data=\"${VOICEBOT_EXTENSION} XML default\"|g" "${PUBLIC_ROUTE}"
fi
...
if [ -f "${DEFAULT_DIALPLAN}" ]; then
    if grep -q "expression=\"^${VOICEBOT_EXTENSION}$\"" "${DEFAULT_DIALPLAN}" 2>/dev/null; then
        ...
        sed -i "s|expression=\"^${VOICEBOT_EXTENSION}$\"|expression=\"^99999$\"|g" "${DEFAULT_DIALPLAN}"
    fi
fi
...
if [ -f "${VOICEBOT_DIALPLAN}" ]; then
    sed -i "s|ws://[^\"]*|${WEBSOCKET_URL}|g" "${VOICEBOT_DIALPLAN}"
fi
